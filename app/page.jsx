...
const contact = {
  whatsapp: '50376048817',
  whatsappDisplay: '+503 7604 8817',
  email: 'ventas@alphalogisticsv.net',
  address: 'Calle 5, Vistas de San Francisco, Antiguo Cuscatlán, La Libertad, El Salvador'
}
...
function Header() {
  return (
    <header className="fixed inset-x-0 top-0 z-50 border-b border-white/10 bg-[#18130c]/95 backdrop-blur-xl">
      <div className="container-alpha flex h-20 items-center justify-between gap-4">
        <a href="#inicio" className="flex items-center gap-3" aria-label="ALPHA LOGISTICS inicio">
          ...
          <div className="leading-none">
            ...
            <p className="mt-1 text-[11px] font-bold tracking-[0.32em] text-amber-200">LOGISTICS</p>
          </div>
        </a>
...
function LuxuryPlaneCard() {
  return (
    <div className="plane-card" aria-label="Avión ejecutivo representando logística aérea premium">
      <div className="plane-orbit" />
      <div className="plane-chip"><Plane size={16} /> Logística aérea SAL</div>
      <div className="lux-plane">
        ...
        <div className="plane-window w1" />
        <div className="plane-window w2" />
        <div className="plane-window w3" />
        <div className="plane-window w4" />
      </div>
      <div className="route-line route-one" />
      <div className="route-line route-two" />
      <div className="plane-panel">
        <p>Operación premium, trato accesible</p>
        <span>Courier, aeropuerto, puerto, zonas francas y DUCA con comunicación directa.</span>
      </div>
    </div>
  )
}
...
function Hero() {
  return (
    <section id="inicio" className="hero-shell">
      ...
      <div className="container-alpha relative grid min-h-[790px] items-center gap-12 pt-24 lg:grid-cols-[1.04fr_.96fr]">
        <div className="fade-up text-white">
          <div className="eyebrow bg-white/10 text-amber-100 ring-white/15"><ShieldCheck size={17} /> Agencia de aduanas en El Salvador</div>
          <h1 className="mt-6 max-w-4xl text-5xl font-black leading-[.98] tracking-tight md:text-7xl">
            Gestión aduanal premium, clara y accesible.
          </h1>
          <p className="mt-7 max-w-2xl text-lg leading-8 text-stone-200 md:text-xl">
            ALPHA LOGISTICS simplifica DUCA, courier, tránsitos, puerto, aeropuerto y zonas francas con atención seria para empresas y personas naturales.
          </p>
          ...
          <div className="mt-12 grid grid-cols-2 gap-3 sm:grid-cols-4">
            {stats.map(([title, label]) => (
              <div key={title} className="rounded-2xl border border-white/10 bg-white/10 p-4 backdrop-blur">
                ...
                <p className="mt-1 text-xs font-semibold text-amber-100/80">{label}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="fade-up space-y-5">
          <LuxuryPlaneCard />
          <div className="rounded-[2rem] border border-white/15 bg-white p-4 shadow-2xl">
            <div className="rounded-[1.5rem] bg-stone-50 p-6 text-stone-900">
              <div className="flex items-center justify-between gap-4 border-b border-stone-200 pb-5">
                <div>
                  <p className="text-xs font-black uppercase tracking-[.22em] text-amber-700">Operación activa</p>
                  <h2 className="mt-2 text-2xl font-black text-[#20170e]">Cobertura nacional clave</h2>
                </div>
                <Globe2 className="text-amber-700" size={36} />
              </div>
              <div className="mt-6 space-y-3">
                {['Puerto de Acajutla', 'Aeropuerto Internacional SAL', 'DHL Santa Elena', 'FedEx Quezaltepeque', 'Todas las zonas francas'].map((item) => (
                  <div key={item} className="flex items-center gap-3 rounded-2xl bg-white p-4 shadow-sm ring-1 ring-stone-200">
                    <MapPin className="shrink-0 text-amber-700" size={20} />
    ...
    <section id="servicios" className="section-padding bg-white">
      <div className="container-alpha">
        <div className="section-head">
          ...
          <p>Un sitio serio no tiene que sentirse inaccesible. Mostramos servicios, puntos operativos y precios base de forma directa.</p>
        </div>
...
function Rates() {
  return (
    <section id="tarifas" className="section-padding bg-stone-100">
      <div className="container-alpha grid gap-10 lg:grid-cols-[.82fr_1.18fr] lg:items-center">
        <div>
          ...
          <h2 className="mt-4 text-4xl font-black tracking-tight text-[#20170e] md:text-5xl">Precios visibles. Cotización sin vueltas.</h2>
          <p className="mt-5 text-lg leading-8 text-stone-600">Tarifas base para operaciones frecuentes. Si tu empresa mueve volumen mensual, armamos un paquete a la medida.</p>
...
function Corporate() {
  ...
  return (
    <section id="corporativo" className="section-padding bg-[#20170e] text-white">
      <div className="container-alpha grid gap-10 lg:grid-cols-2 lg:items-center">
        <div>
          <div className="eyebrow bg-white/10 text-amber-100 ring-white/15"><Users2 size={17} /> Soluciones corporativas</div>
          ...
          <p className="mt-5 text-lg leading-8 text-stone-200">Creamos paquetes mensuales para operaciones recurrentes en zonas francas, courier, puerto y aeropuerto. La idea es que tu equipo tenga un punto de contacto confiable y tiempos más predecibles.</p>
        </div>
        <div className="rounded-[2rem] bg-white p-6 text-stone-900 shadow-2xl">
          <div className="rounded-[1.5rem] bg-stone-50 p-6">
            {items.map((item) => (
              <div key={item} className="flex items-center gap-4 border-b border-stone-200 py-5 last:border-0">
                <BadgeCheck className="text-amber-700" size={24} />
                <p className="font-black text-[#20170e]">{item}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="container-alpha grid gap-10 lg:grid-cols-[.95fr_1.05fr] lg:items-center">
        <div className="rounded-[2rem] bg-stone-100 p-6">
          <div className="rounded-[1.5rem] bg-white p-7 shadow-sm">
            <ClipboardCheck className="text-amber-700" size={42} />
            <h3 className="mt-5 text-2xl font-black text-[#20170e]">Nuestra misión</h3>
            <p className="mt-3 leading-8 text-stone-600">Hacer que los procesos aduanales sean más simples, rápidos y transparentes para quienes importan, exportan o reciben paquetería internacional en El Salvador.</p>
          </div>
        </div>
        <div>
          <div className="eyebrow"><CheckCircle2 size={17} /> Nosotros</div>
          <h2 className="mt-4 text-4xl font-black tracking-tight text-[#20170e] md:text-5xl">Seriedad corporativa, trato humano.</h2>
          <p className="mt-5 text-lg leading-8 text-stone-600">ALPHA LOGISTICS trabaja con enfoque en rapidez, cumplimiento y eficiencia. Atendemos desde una persona natural que necesita liberar un paquete hasta empresas que requieren operación mensual con procesos definidos.</p>
...
function Contact() {
  return (
    <section id="contacto" className="section-padding bg-stone-100">
      <div className="container-alpha grid gap-10 lg:grid-cols-[.85fr_1.15fr]">
        <div>
          <div className="eyebrow"><Mail size={17} /> Contacto</div>
          <h2 className="mt-4 text-4xl font-black tracking-tight text-[#20170e] md:text-5xl">Cuéntanos qué necesitas mover o liberar.</h2>
          <p className="mt-5 text-lg leading-8 text-stone-600">Responderemos con una ruta clara: trámite, requisitos, costo base y siguiente paso.</p>
          <div className="mt-8 space-y-3">
            <a className="contact-line" href={`https://wa.me/${contact.whatsapp}`}><MessageCircle size={20} /> {contact.whatsappDisplay}</a>
            <a className="contact-line" href={`mailto:${contact.email}`}><Mail size={20} /> {contact.email}</a>
            <div className="contact-line"><MapPin size={20} /> {contact.address}</div>
          </div>
        </div>
        <form className="rounded-[2rem] bg-white p-6 shadow-xl ring-1 ring-stone-200">
          ...
          <p className="mt-4 text-center text-xs font-semibold text-stone-500">Formulario visual listo. Para producción se puede conectar a Formspree, Resend, HubSpot o API propia.</p>
        </form>
      </div>
    </section>
  )
...
export default function Page() {
  return (
    <main>
      ...
      <About />
      <Contact />
      <footer className="bg-[#18130c] py-8 text-center text-sm font-semibold text-stone-300">
